...
pub fn main() !void {
    ...
    const buffer = try std.fs.cwd().readFileAlloc(allocator, "input/day4", std.math.maxInt(usize));
    defer allocator.free(buffer);

    const width = std.mem.indexOfScalar(u8, buffer, '\n') orelse return error.NotFound;
    const height = try std.math.divCeil(usize, buffer.len, width + 1);
                ...
                inline for (offs) |dy| {
                    inline for (offs) |dx| {
                            ...
                            (ny >= 0 and ny < height))
                        {
                            ...
                            neighbors += @intFromBool(buffer[idx] == '@');
